...
allPossibleIntegers = range(0, 28124)
abundantNumbers = []
...
def check_abundant_sums(number):
    for abundant_number in abundantNumbers:
        if abundant_number + number < len(allPossibleIntegers):
            allPossibleIntegers[abundant_number + number] = 0


for i in xrange(1, 28124):
    if is_abundant_number(i):
        abundantNumbers += [i]
        check_abundant_sums(i)

print str(sum(allPossibleIntegers))